  ...
  if (star == "") {
    ...
    try {
      ...
      let alreadyRated = product.ratings.find(
        (rating) => rating.postedby.toString() === findUser._id.toString()
      );
      if (alreadyRated) {
      ...
      let ratingsum = _product.ratings
        .map((item) => item.star)
  ...
  try {
    ...
    for (let i = 0; i < cart.products.length; i++) {
      if (cart.products[i]._id == id) {
        const filterCart = cartUser.filter(
          (product) => product._id != cart.products[i].product.toString()
        );
...
const applyCoupon = asyncHandler(async (req, res) => {
  ...
  if (validCoupon === null) {
    ...
    if (user.userDiscount > 0) {
      totalAfterDiscount = (
        cartTotal -
        (cartTotal * validCoupon.discount * user.userDiscount) / 100
      ).toFixed(2);
    } else {
      totalAfterDiscount = (
        cartTotal -
